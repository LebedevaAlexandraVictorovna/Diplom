...
def update_file(students, admins):
    ...
    obj = s3.Object('liststudentsadmins','list.txt')
    data = ""
    for stud in students:
        data = data + stud.get_surname() + "*" + stud.get_name() + "*" + stud.get_patronym() + \
                "*" + stud.get_login() + "*" + stud.get_password() + "*" + str(
            stud.get_course()) + "*" + stud.get_group() + "*" + str(stud.get_subgroup()) + "\n"
    for adm in admins:
        data = data + adm.get_surname() + "*" + adm.get_name() + "*" + adm.get_patronym() + \
...
def show_rating(students, disciplines):
    ...
    for st in students:
        ...
        avg = 0  # средний балл
        gpa = 0
        for ob in disciplines:
            if int(ob.get_course()) <= int(st.get_course()):
                n = n + 1
                avg = avg + int(ob.find_mark(str(st.get_surname())))
                cr = cr + int(ob.get_credits())
                gpa = gpa + int(ob.get_credits()) * int(ob.find_mark(str(st.get_surname())))
        avg = avg / n
        gpa = gpa / cr
...
def handle_client(client):  # Takes client socket as argument.
    ...
    obj2 = s3.Object('liststudentsadmins','disciplines.txt')
    text = obj2.get()['Body'].read().decode('utf-8').splitlines()
    disciplines = []
    for line in text:
        spl = line.split("*")
        subj = Discipline()
            ...
            if msg1 == "1":  # внести изменения в учетную запись
                ...
                if flag == "1":
                    ...
                    if z == "1":  # меняем фамилию
                        ...
                        for subj in disciplines:
                            ...
                            obj = s3.Object('liststudentsadmins',subj.get_vedomost())
                            text = obj.get()['Body'].read().decode('utf-8').splitlines()
                            new_lines = []
                            for line in text:
                                if surname in line:
                ...
                for stud in students:
                    if stud.get_surname() == surname:
                        ...
                        for ob in disciplines:
                            if int(ob.get_course()) <= int(stud.get_course()):
                                msg = msg + "\n" + str(ob.get_name()) + " " + str(ob.find_mark(str(stud.get_surname())))
